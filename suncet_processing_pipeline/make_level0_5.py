...
def _build_argument_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Canonical staged Level 0.5 ingest builder: merge raw files, clean wrappers, "
...
def run(
    ...
    *,
    ...
    """Run Level 0.5 without creating a provenance wrapper.

    ``main`` is the standalone entry point and wraps this function in provenance.
    The top-level multi-level processor calls ``run`` inside its own provenance
    context and may supply an already parsed config and resolved data folder.
    """
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not folder.is_dir():
        return run(argv_list, _prepared=(config, folder))
    ...
    with provenance:
        ...
        run(argv_list, _prepared=(config, folder))

...
Obtain the APID 1 packet-length answer and a flight-equivalent RF recording so
the waveform and SatNOGS decoder-input boundary can replace the provisional
assumptions. In parallel, review the offline SatNOGS DB submission draft. No
external DB suggestions should be submitted until the public source document
is reviewed.

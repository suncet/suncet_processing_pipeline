#!/usr/bin/env python3
"""
Ingest a raw **EM / CSIE** byte capture with CCSDS ``1A CF FC 1D`` frame sync markers
between space packets, strip initial ``0xFF`` filler, collect **APID 536 (csie_data)**
uncompressed row payloads, and write an **inferno** PNG (origin upper; no extra rotation for now).

This script does not load CTDB / ``gen_pkts``; it only needs the well-known frame layout
and default SunCET full-frame size (2000×1504, uint16 big-endian per row, CCSDS
``sequence_number`` = 1-based row index).

Optionally loads a **reference FITS** test pattern and writes **parsed − reference** statistics,
a single-panel inferno **\_diff** PNG, and a 1×3 **\_parsed\_ref\_diff** figure (parsed, reference, difference).
"""
...
_DEFAULT_INPUT = (
    ...
    "2026-04-24_em_csie_test_pattern_over_xband/parsed_output_raw_8p3mbps"
)
...
def main() -> None:
    p = argparse.ArgumentParser(
        description="Strip 0xFF, split on CCSDS sync, build CSIE image(s) from APID-536, save PNG"
    )
    p.add_argument(
        "input",
        nargs="?",
